...
Versions 1.2 and 1.3 of this document were
developed in part with support from the
German federal ministry for research and education's e-inf-astro project (BMBF
FKZ 05A17VH2 and 05A23VH4).
...
H$\boldsymbol\alpha$
line taken in the second half of 2015?}  In the current
version of VODataService,
the \xmlel{spatial}, \xmlel{spectral}, and \xmlel{temporal}
...
query'') that has been in use since the beginning of the VO is no longer
...
of an actual object on the one hand, or completely artificial data
...
\paragraph{I have found a table containing interesting data.  How do I
query it?} Here, researchers have used data discovery queries to find,
for instance, a table containing data for objects out to redshift 8.
This comes in a resource with five tables and eight capabilities – which
capability can they use to query the table they found?  VODataService
1.3 offers the capability type CapabilityForTable that allows clients
to narrow down a resource's capabilities by the table(s) they operate on.
...
pairs.  See \citep{note:caproles} for a longer treatment of the problem.
...
An additional resource class defined by VODataService 1.1,
\xmlel{vs:DataCollection}
has been deprecated since version 1.2.
Resource record authors are requested to migrate or discard resource
records using this deprecated type.  The \xmlel{vs:StandardSTC} resource
class defined in 1.1 has been
...
although at this point no obvious use cases for DataResource
have been identified;
it was included mainly for symmetry with CatalogResource.

A few examples may help illustrating the intended systematics:

\begin{itemize}
\item A set of spectra with an SSA service: CatalogService, because
there is a relational model and a dedicated service
\item Some tables queriable via TAP: CatalogResource, with an auxiliary
TAP capability and an isServedBy relationship to the main TAP service
\item An obscore table: CatalogResource, for analogous reasons
\item A TAP-queriable table with a custom SCS service: CatalogService,
because there is a dedicated service; the question to ask would be
something like: in a global enumeration of SCS services, should this one
be counted?
\item A VOEvent stream: DataService, because VOEvent is not actually
relational.
\item An rsync service to pull observations of a given instrument:
DataService, because again there is no involvement of tabular data
(worth mentioning).
\item A service-validating service accepting the an access URL:
DataService.
\end{itemize}
...
declare the metadata of the table(s) making up the data
...
an element of type \xmlel{stc:STCResourceProfile}\footnote{Defined in
...
\item[Meaning] When present, the MOC is written in a non-celestial (e.g.,
                     ...
                     ICRS must be used.
\item[Occurrence] optional
\item[Comment] VODataService 1.3 does not prescribe a vocabulary for
...
\subsubsection{Statistics for Columns and Params Derived From Them}
...
\subsubsection{Data Types in BaseParam}
...
of an input parameter that might appear inside a
...
\subsection{Capability Type Extension: CapabilityForTable}

A given service may expose specific tables but not others that may be in
a resource's tableset.  For instance, cone searches will be linked to a
specific table in Simple Cone Search 1 \citep{2008ivoa.specQ0222P}, and
HATS \citep{2025hatsnote} endpoints will only deliver data from a single
table.

In particular when in data discovery, clients discover a table rather
than a service, they need to have a way to get from the table to
services that make these tables available, which in multi-table
tablesets may be non-trivial.  Hence, there needs to be a mechanism to
declare which capability exposes data from which table.  This is why
VODataService 1.3 specifies the following derivation from
\xmlel{vr:Capability}:


% GENERATED: !schemadoc VODataService-v1.3.xsd CapabilityForTable
\begin{generated}
\begingroup
        \renewcommand*\descriptionlabel[1]{%
        \hbox to 5.5em{\emph{#1}\hfil}}\vspace{2ex}\noindent\textbf{\xmlel{vs:CapabilityForTable} Type Schema Documentation}

\noindent{\small{}A capability containing one or more references into the
            resource's VODataService tableset.\par}

\noindent{\small{}The typical case here is a cone search just exposing one of
            several tables that are part of a resource.  Clients discovering
            a table can then use forTable attributes to find services
            on top of this table.\par}

\vspace{1ex}\noindent\textbf{\xmlel{vs:CapabilityForTable} Type Schema Definition}

\begin{lstlisting}[language=XML,basicstyle=\footnotesize]
<xs:complexType name="CapabilityForTable" >
  <xs:complexContent >
    <xs:extension base="vr:Capability" >
      <xs:sequence >
        <xs:element name="forTable" type="xs:token" minOccurs="0"
                  maxOccurs="unbounded" />
      </xs:sequence>
    </xs:extension>
  </xs:complexContent>
...
\vspace{0.5ex}\noindent\textbf{\xmlel{vs:CapabilityForTable} Extension Metadata Elements}

\begingroup\small\begin{bigdescription}\item[Element \xmlel{forTable}]
\begin{description}
\item[Type] string: \xmlel{xs:token}
\item[Meaning] A reference into the tableset giving one table
                        exposed by this capability.
\item[Occurrence] optional; multiple occurrences allowed.
\item[Comment] This must be exactly a string as found in a
                        table's name child.  In particular, if there are
                        quotes or similar syntactic features in the name,
                        this must be given here, too.
...
This type can be used as-is.  Protocols requiring extra metadata can
...
\item Adding a type \xmlel{vs:CapabilityForTable} for linking tables
and capabilities.
\end{itemize}
