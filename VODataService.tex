...
relationship between tables and capabilities.  This is because the
uniqueness requirements on the \xmlel{id} attribute (cf.~sect.~3.3.1 in
\citet{std:XML}) is very hard to maintain when resource
records are frequently combined into joint documents, for instance,
while building OAI-PMH responses.  Also, exploiting metadata of
this type in discovery usually leads to ugly query patterns.  Finally,
id references would break when capabilities and tables come in two
...
\xmlel{stats} element of an input parameter, or in dedicated elements
